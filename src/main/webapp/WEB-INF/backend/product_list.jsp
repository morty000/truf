    ...
    $(function () {
        ...
        function build_table_data(result) {
            ...
            $.each(products, function (index, item) {
                ...
                var status = null;
                if (item.status == 1) {
                    status = $("<td class='td-status'><span class='label label-success radius'>已发布</span></td>");
                } else if (item.status == 2) {
                    status = $("<td class='td-status'><span class='label label-defaunt radius'>已下架</span></td>");
                } else if (item.status == 3) {
                    status = $("<td class='td-status'><span class='label label-defaunt radius'>已删除商品</span></td>");
                }
                ...
                if (item.status == 1) {
                    ...
                    var edit1 = $('<a style="text-decoration: none" href="javascript:;" title="发布" class="edit2"><i class="Hui-iconfont">&#xe603;</i></a>');
                }
                if (item.status != 3) {
        ...
        $("#search_btn").click(function () {
            ...
            function search_btn(pageNum, pageSize) {
                $.ajax({
                    type: "post",
                    url: "${ctx}/product/time_keywords.do",
                    data: {
                        "pageNum": pageNum,
                        "pageSize": pageSize,
                        "startTime": startTime,
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            if (result.msg == "修改商品销售状态成功") {
                                thisObj.parents("tr").find(".td-status").html(
                                    '<span class="label label-defaunt radius">已删除商品</span>');
                                thisObj.parents("tr").find("td:eq(6)").find("a").remove();
                                layer.msg('已删除!', {
                                    icon: 1,
                                    time: 1000
                                });
                ...
                success: function (result) {
                    if (result.msg == "修改商品销售状态成功") {
                        thisObj
                            ...
                            .find(".td-manage")
                            .prepend(
                                '<a style="text-decoration:none" href="javascript:;" title="发布" class="edit2">' +
                ...
                success: function (result) {
                    if (result.msg == "修改商品销售状态成功") {
                        thisObj
                            ...
                            .find(".td-manage")
                            .prepend(
                                '<a style="text-decoration:none" href="javascript:;" title="下架" class="edit1">' +
                                '<i class="Hui-iconfont">&#xe6de;</i></a>');
                        thisObj
                            .parents("tr")
                            .find(".td-status")
